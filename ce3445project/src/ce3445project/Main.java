    ...
    public static void main(String[] args) throws Exception {
        ...
        if (size == 0){
            ...
            for(int i=0; i<arr.length; i++){
                arr[i] = (String.valueOf((int)Math.floor(Math.random()*100)));
            }
            System.out.println("Input is empty, using random array: ");
            for(String s : arr){
                System.out.print(s + " ");
            }
            System.out.println();
        ...
        list2.addAll(list);
        list = ms.triSort(list);
        for(int i=0;i<list.size();i++){
            System.out.println(i+1+":\t" +list.get(i));
        }
        System.out.println("Comparisons for all merging: " + ms.comparisons);
    }
